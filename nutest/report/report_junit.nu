...
export def create [path: path]: nothing -> record<name: string, save: closure, results: closure> {
    {
        name: "report junit"
...
export def "to junit" []: table<suite: string, test: string, result: string, output: list<any>> -> string {
    testsuites | to xml --self-closed --indent 2
}
...
def testsuite []: table<suite: string, test: string, result: string, output: list<any>> -> record {
    ...
    let suite = $rows | first 1 | get suite
    let stats = $rows | count
    {
        tag: testsuite
        attributes: {
